...
contract AskMiUltimate {
    ...
    mapping(address => uint256) internal tokensLookup;
    ...
    constructor(
        ...
        require(_tip > 0, "The tip cost must be greater than 0.");
        ...
        // Create an empty token needed
        // to perform lookups and accept eth
        TokenPrice memory _token;

        _token.tokenAddress = address(0);
        _token.tip = _tip;
        _token.tiers = _tiers;

        // Occupy the first token index for eth
        tokenPrices.push(_token);
    ...
    // Consider removing TipIssued event
    event TipIssued(
        address _tipper,
        address _questioner,
    ...
    {
        return tokenPrices[tokensLookup[_tokenAddress]].tiers;
    }

    // @notice Get the complete tiers array
    function getTip(address _tokenAddress) public view returns (uint256) {
        return tokenPrices[tokensLookup[_tokenAddress]].tip;
    }
    ...
    function updateTip(address _tokenAddress, uint256 _newTipPrice)
        public
        onlyOwner
    {
        require(_newTipPrice > 0, "The tip price must be greater than 0.");

        uint256 index = tokensLookup[_tokenAddress];
        TokenPrice storage _token = tokenPrices[index];

        // Update token tip price
        _token.tip = _newTipPrice;

        // Save changes
        tokenPrices[index] = _token;
    }
    ...
    function addERC20(
        ...
        require(
            tokensLookup[_tokenAddress] == 0 && _tokenAddress != address(0),
        ...
        // Assign the array index to the current address
        tokensLookup[_tokenAddress] = tokenPrices.length;
        // Save the new token and its prices
        tokenPrices.push(_tokenPrice);
    }
    ...
    function ask(
        ...
        uint256[] memory _tiers = tokenPrices[tokensLookup[_tokenAddress]]
            .tiers;
        ...
        noReentrant
    {
        ...
        address _tokenAddress = _exchanges[_exchangeIndex].tokenAddress;
        uint256 _tip = tokenPrices[tokensLookup[_tokenAddress]].tip;
        // If _tokenAddress is the 0 address, this is an ETH transaction
        if (_tokenAddress == address(0)) {
            // Check that the tip amount is correct
            require(msg.value == _tip, "The tip amount is incorrect.");
            ...
            require(
                _token.transferFrom(msg.sender, owner, _tip),
